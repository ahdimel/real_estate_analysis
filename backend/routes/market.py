...
@router.get("/rate")
def market_rate_status(_current_user: User = Depends(get_current_user)):
...
def mortgage_rate(
    ...
    rate, label, is_stale = get_mortgage_rate(db)
    return {"rate_pct": rate, "label": label, "is_stale": is_stale}
