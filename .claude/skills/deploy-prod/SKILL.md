...
- **`SECRET_KEY` must be ≥ 32 characters in Railway env vars**: the backend now asserts `len(SECRET_KEY) >= 32` at startup and refuses to boot otherwise. Generate a compliant key: `python -c "import secrets; print(secrets.token_hex(32))"`. Set it under Railway backend service → Variables → `SECRET_KEY`.
- **`/docs` returns 404 in production — this is intentional**: OpenAPI Swagger UI, ReDoc, and the `/openapi.json` schema are disabled when `RAILWAY_ENVIRONMENT` is set (L2 security fix). Do not treat a 404 on `/docs` as a deploy failure. They are still available at `http://localhost:8000/docs` locally.
