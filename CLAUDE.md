...
- **Duplicate-analysis prevention is frontend-only**: `inputsUnchanged` in `frontend/app/properties/[id]/analysis/page.tsx` disables the "Run Analysis" button when all 21 analysis-relevant fields match the latest snapshot. This is a UX guard, not a backend rule — the API will still accept a POST if called directly. If credits ever become paid, add a backend fingerprint check at that point. `ANALYSIS_FIELDS` in that file is the authoritative list of which fields matter; update it if you add a new field to the analysis engine.
